...
function isAdmin(req) {
  return parseCookies(req).openPoblaAdmin === adminToken();
}

function setAdminCookie(res) {
  res.setHeader("Set-Cookie", `openPoblaAdmin=${encodeURIComponent(adminToken())}; HttpOnly; SameSite=Lax; Path=/`);
}

function clearAdminCookie(res) {
  res.setHeader("Set-Cookie", "openPoblaAdmin=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0");
}
...
function sendUnauthorized(res) {
  sendJson(res, 401, { error: "Cal iniciar sessio com administrador" });
}

function isProtectedPage(urlPath) {
  return ["/admin.html", "/index.html", "/quadrant-control.html", "/quadrant-admin.html", "/jugadors.html"].includes(urlPath);
}
...
function serveStatic(req, res) {
  ...
  if (isProtectedPage(urlPath) && !isAdmin(req)) {
    sendRedirect(res, `/login?next=${encodeURIComponent(new URL(req.url, `http://${req.headers.host}`).pathname)}`);
    return;
  }
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && url.pathname === "/api/login") {
    try {
      ...
      if (String(body.password || "") !== ADMIN_PASSWORD) {
        sendJson(res, 403, { error: "Contrasenya incorrecta" });
        return;
      }
      setAdminCookie(res);
  ...
  if (req.method === "GET" && url.pathname === "/api/export") {
    ...
    res.writeHead(200, {
      "Content-Type": "application/json; charset=utf-8",
      "Content-Disposition": `attachment; filename="open-la-pobla-${new Date().toISOString().slice(0, 10)}.json"`,
      "Cache-Control": "no-store",
    });
  ...
  const protectedPostRoutes = [
    ...
    "/api/players",
    "/api/bracket-manual",
    "/api/bracket-load-match",
    "/api/bracket-unassign-table",
    "/api/bracket-random-tables",
  ];
  if (
    req.method === "POST" &&
    (url.pathname.startsWith("/api/scoreboards/") || protectedPostRoutes.includes(url.pathname)) &&
    !isAdmin(req)
  ) {
    sendUnauthorized(res);
    return;
